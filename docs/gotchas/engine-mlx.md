...
### Every sampled token ranked the whole vocabulary, and a shortlist only agrees with it if both sides break ties the same way
`applyTopP` ranked 248,320 logits on every sampled token and `applyTopK` paid a second full pass. On Metal `Partition::eval_gpu` and `ArgPartition::eval_gpu` both say "we direct partition to sort for now", so `mlx_topk` and `mlx_argpartition` ARE that same multi-block merge sort — a shortlist built from `argpartition` buys nothing here. M4 Max, Qwen3.8-Flash-Next, MTP off: 65.0 tok/s at temperature 0 against 60.5 at temperature 1 with top_p 0.95 / top_k 20.

`topRanksDescending` takes the exact top `m` values AND their column ids with no full-row rank. Chunk the row; every element of a chunk is at most that chunk's max, so a chunk whose max is not among the `m` largest chunk maxima holds no top-`m` element. At most `m` chunks can, so one small sort of `V / chunk` chunk maxima plus one of `m * chunk` candidates is exact — balanced at `chunk = sqrt(V / m)`. `applyTopK` scatters the shortlist's ids; `applyTopP` reads its cutoff off the k-shortlist, because nothing below the row's top `k` can be the cutoff once top-k has run.

Under the rank contract (ties cut by rank, not value) exactness is a question about ORDER, and three things had to line up:
- **One tie rule, shared by both routes.** `ranksDescending` is a stable ascending argsort of the NEGATED row: value descending, ties by lowest column id — the `msv_qsa_select` rule. mlx's merge sort is stable (`ThreadSort` swaps only on a strict less-than; the merge takes from B only when `b < a`, and A holds the lower ids), which is an undocumented property of a pinned dependency, so a test asserts the order directly.
- **The chunk pick must take the LOWEST ids among equal maxima.** A tie group's contributing chunks are a PREFIX of its ascending-id list, since a lower chunk id holds a lower column id, which outranks. Ranking the chunk maxima descending gets this for free; taking the last `m` of an ascending argsort gets it exactly backwards and a 248k all-equal row then keeps the wrong columns (caught as `mismatch at 59: 244922 vs 20730`).
- **Candidates are laid out in ascending column order** (sort the winning chunk ids before gathering), so the stable rank over `m * chunk` candidates breaks ties the way the whole row would.

The nucleus is the mass STRICTLY above each rank — an exclusive scan down the ranking — which is a function of the shortlist alone, because everything outranking a top-`k` column is itself top-`k`. Rank 0 sees zero mass, so the argmax is always kept. The normalizer is the whole row's `logsumexp`, not the shortlist's softmax (mlx's logsumexp accumulates in f32 whatever the input dtype), and the scan runs in f32: mlx instantiates cumsum at the INPUT dtype (`sum_bfloat16_bfloat16`) and logits off a quantized `lm_head` are bf16. A bf16 accumulator dropped 148 of 3720 nucleus columns on one of six probe rows, and worse, made the running sum depend on how many entries the scan walked — the one thing a shortlist changes. On the CPU's sequential scan the same row keeps 778 against an exact 4743; the GPU's blocked scan hides most of it, which is why only some rows show the defect.

Bar: the shortlist route and ranking the whole row produce BYTE-identical filtered logits and draw the same token under the same key. Pinned over `[m, V]` and `[B, L, V]` blocks at V = 8192 and 248,320, six `(top_p, top_k)` settings, a row quantized to 0.25 steps, and a row where every column is identical. `applyTopK` keeps EXACTLY k. The f64 nucleus reference is allowed to differ by one column: the boundary column's own probability is ~1e-5 while the f32 scan carries ~1e-7 of error, so the exclusive mass can cross `top_p` one column early. Mutation-checked: dropping the f32 promotion fails the nucleus test, and picking the highest chunk ids among tied maxima fails both ranking tests.
