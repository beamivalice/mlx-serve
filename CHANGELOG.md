...
- Qwen 3.8 Flash Next aborted the whole process on every generated token once a conversation passed 2,052 tokens, whenever sparse attention was running its dense-mask arm (batched slots, and any context where the block gather is off). An optimization that skips the sparse-attention visibility work where it is provably a no-op handed a null array to one consumer that had not been told about the skip; mlx-c aborts on null. Introduced and fixed within this release.
