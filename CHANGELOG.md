...
- **A cached conversation survives on SSD instead of being rebuilt, on Flash Next.** At the long contexts this model is for, the machine has room for the weights plus *one* conversation's attention state and nothing else — so anything the cache could not keep in memory used to be re-read from scratch, several minutes of work per switch. The SSD tier now holds the full conversation rather than only the part that fitted in memory: chunks are written as the prompt is processed rather than all at the end, the recurrent-state checkpoints that make an entry usable are written alongside them instead of queued behind them, and the writing itself moved off the thread that runs the model, so a large save no longer delays the next request. Between requests, memory keeps the conversation being served and every other one moves to disk. Cache files written by earlier versions are still readable, and files written now are readable by earlier versions. `MLX_SERVE_PREFIX_SSD_FIRST=0` restores the previous behaviour; nothing changes for any other model.
- **`--prefix-cache-mem` means something different on Flash Next (only).** On that model it is now the allowance for conversations *other* than the one being served — memory always keeps the active conversation, which costs nothing extra because it is the same data the model is already using. The default (no flag) keeps one conversation in memory and puts the rest on SSD. At long context this is strictly better: the old accounting counted the active conversation twice and left less than half of one entry, so warm turns re-read the prompt anyway. At short context it does mean fewer conversations held in memory than before. The resolved figure is written to the log at startup, naming the flag.
...
- **Long prompts on Flash Next prefill and decode faster.** The sparse-attention machinery around the 12 full-attention layers rebuilt work every layer and every round that only depends on the context length: the block constants, the pooled block positions, the pooled key bank, and the indexer history (which was re-concatenated in full on every chunk). Those are now built once and appended to, and the top-512 block selection is a single fused kernel per row instead of a nine-dispatch chain. On an M5 Max at 8-bit KV, measured on a 64k/128k/256k/374k ladder: prefill on new tokens +1.7/+6.5/+19.5/+20.9%, time-to-first-token -1.7/-6.1/-16.3/-17.3%, and ordinary decode +6.5/+8.7/+15.3/+23.1% (38.0 -> 46.8 tok/s at 374k). Short contexts do not regress. The fused selection also fixes a correctness detail: the old chain broke ties between equally-scored blocks by a tiny numeric bias that stops working once scores grow past about 0.84, so on real scores it never implemented the reference's rule at all; the kernel implements it exactly.
- **Speculative verification on Flash Next reads long contexts through one fused kernel.** Checking a batch of draft tokens against a long prompt used to gather the selected key blocks into a padded scratch buffer and run ordinary attention over it. That work is now a single kernel that reads the packed keys and values in place, split across the key range so the GPU has something to do on every core — the first version gave each query row one thread group, which left 12 of 40 cores busy and ran SLOWER than the gather it replaced. At 62k tokens on an M5 Max a verify forward is about 13% faster at width 6 and 8% at width 3. `MLX_SERVE_QSA_ATTN_KERNEL=0` restores the gather; `MLX_SERVE_QSA_ATTN_NSPLIT` (default 16) and `MLX_SERVE_QSA_ATTN_MIN_S` (default 2) set the split count and the lowest width the kernel serves.
- **A prefix-cache hit no longer leaves Flash Next's draft head guessing.** The model's own draft head keeps a history of what it has already seen. That history was not saved with the cached prefix, so every cache hit restored the conversation's attention state and left the head with nothing — it kept drafting, the drafts kept being rejected, and speculation quietly collapsed to ordinary decoding (accepted tokens per round fell from 1.59 to 0.59 on a 62.7k prose prompt). Both halves are now saved and restored together, on disk as well as in memory; a cache entry that has only one half is treated as a miss rather than used. `MLX_SERVE_MTP_HEAD_PERSIST=0` restores the old behaviour. Cached prefixes written by earlier versions are still readable.
- **The n-gram table's long-prompt reads go through the same prefetch pool as decode.** Flash Next reads scattered rows out of a 32 GB memory-mapped table; during a long prefill those reads were still a serial walk, which on a cold table means one SSD fault at a time. They now use the parked reader pool decode has used since 26.9.1. On by default; `QWEN4_PLE_PREFETCH_PREFILL=0` restores the serial walk, and the log says which arm ran.
- **Speculative decoding now switches itself off where a plain step is faster, and back on when it is not.** A speculative round reads the whole context back, so past some conversation length it costs more than the ordinary steps it replaces — on long prose that point arrives well before anything else notices, because the drafts are still being accepted. The server now measures what a plain decode step costs at each context length, compares it with what the round it is about to run costs, and decodes normally for the rest of the reply when the round loses. Nothing to configure: the measurements are the ones it already takes, and they persist per machine and model. This is now the normal way speculation stops; `--max-mtp-ctx` below stays as the hard override for when you want a fixed ceiling instead of a measured one.
- **New `--max-mtp-ctx <n>`: keep speculative decoding off past a context length you choose.** A speculative round reads the whole context back, so on a long conversation it can cost more than the ordinary steps it is trying to replace — and on prose it often does. Pass `--max-mtp-ctx 16384` and any request whose prompt is longer decodes normally, while a reply that grows past the limit mid-answer switches over for the rest of that answer. Off by default (no limit), and it applies only to the model's own draft head, not to the other speculative paths. It is a hard override: below the limit the measured switch above still decides, above it speculation stays off regardless of what the measurements say, and a request cannot lift it by asking for speculation explicitly.

### Fixes
- A warm turn on Flash Next copied its whole cached conversation before writing a single token. Restoring a cached prefix hands the conversation's attention buffers to the request, but the cache entry goes on holding them too — and MLX only writes into a buffer in place when nothing else refers to it, so the first token of every warm turn quietly duplicated the entire prefix: 5.13 GB and about 110 ms at 393k tokens, roughly 45% of the wait before the first reply token, and invisible to every counter because a copy-on-write is not a growth. On a full-prefix hit the cache now hands the buffers over outright and takes the entry out of circulation until the turn commits or is abandoned, so the append writes in place. A second, unrelated cost went with it: the sparse-attention history was re-seeded into a tight buffer that had to grow one line later, two full passes over it (about 81 ms at 384k). `MLX_SERVE_RESTORE_MOVE=0` restores the copy; partial hits are unchanged, and Flash Next only.
- A warm turn on Flash Next was billed for memory it already had. The admission estimate charges a request for the attention cache it is about to allocate, and it charged for the restored prefix too, even though the restore had just handed the request the buffers holding it — about 12 GB of double counting at a 786k-token context, on a check whose failure is an out-of-memory abort rather than a refusal. The estimate now credits the rows the restore really hands over: the key/value cache only, and only while the request fits inside the buffers that arrived, because a request that outgrows them allocates a whole new buffer beside the cached entry's, which does not go away. The sparse-attention history is deliberately not credited — it is copied on arrival, so the request does allocate it. A request whose warm prefix the connection thread cannot see is decided by the thread that can, rather than refused early.
- Qwen 3.8 27B packs, and every other model with a separate draft head, lost about 25% of decode speed at 8k against 26.9.1. The planner was not the cause and is byte-for-byte what 26.9.1 shipped; what changed is that the file it records measured round costs into was given a new name, so every boot started from an empty table — and while the table is empty the planner keeps its exploring valve open and runs every round at the wider, more expensive shape (37 of 40 rounds at 77 ms, against a warm table's 1 of 52 at 51 ms). That file's layout now belongs to the architecture rather than to the build: a model with a sidecar draft head reads and writes exactly the file 26.9.1 wrote and starts warm again, and Flash Next's own newer layout warm-starts from the old file for the context buckets the two share, dropping the one bucket whose meaning changed rather than mis-assigning it. Two smaller costs went with it — a model that never reads the serial timing row no longer takes that timing on every decoded token and rewrites the file at the end of every request for it, and the probe that fills the last input the decision needs is now spent only when it really is the last one missing.
- Long conversations on Flash Next reserved memory for a copy of the sparse-attention index that no longer exists. The index keeps a history of raw keys beside the attention cache; committing a conversation to the cache used to duplicate it, so the conversation being served and the cached copy held the same bytes twice — and the admission estimate priced only one of them, which is the wrong error in the direction that aborts. The newest cached entry now shares the live buffer instead of copying it, and the estimate prices what is really held: the reservation at a 1M context drops from 20,736 to 18,432 MiB, and measured memory during decode falls 3-4 GB on 786k-1M prompts. `MLX_SERVE_QSA_HISTORY_SHARE=0` restores the copy, and the estimate follows it.
- A cached conversation on SSD was re-written in full on every turn. An entry is saved when a reply finishes, so its last tokens are that reply; the next prompt contains the reply and then diverges inside it, and the extend path — which only ever appends after an exact prefix — found nothing it could keep and wrote several hundred megabytes again for bytes that were already on disk. Whole chunks under the common prefix are now shared with the entry they came from rather than copied, and the disk accounting counts each file once, so the budget, the sweep and the eviction cannot disagree about what is stored. `MLX_SERVE_SSD_CHUNK_SHARE=0` restores the copy. Flash Next only.
- Saving a conversation to SSD as it was read back slowed down the very turns it was meant to speed up. On Flash Next the cache writes each chunk of the prompt to disk while the prompt is still being processed, and that write sat in the time before the first reply token: a warm turn paid an extra 0.18 s at 16k, 0.36 s at 32k and 0.74 s at 64k. Two things were wrong. The write pushed each tensor to the GPU on its own, about 2,300 round trips on a 32k turn, which made a fixed per-tensor cost look like slow disk (1.13 GB/s against the 7-8 GB/s the same data reaches on the same machine); it is now one batched pass per chunk file, writing byte-identical files. And a turn whose new text is shorter than one chunk no longer writes through at all: there is no chunk-aligned progress for it to save, and the save that happens after the reply is sent covers it anyway. Warm turns are back to full speed and nothing on disk changed format.
- Two conversations of very different length could be decoded together on Qwen 3.8 Flash Next and the other hybrid architectures, which made the short one build a key/value tensor as wide as the long one's on every generated token — several gigabytes per token that nothing accounts for, and an out-of-memory abort under concurrent long-context load. A cap for exactly this has existed since 26.8, but it read a token count that these architectures never advance: it saw zero for every conversation, computed no waste, and so never once fired. It now reads the length from the attention layers themselves, which is right on every architecture, and the log line names the waste ratio it compared instead of only its verdict. Affects Qwen 3.5 and 3.8 (including Flash Next), LFM2, Nemotron-H and Ling 3.0; dense architectures are unchanged, and on the affected ones a pair of similar-length conversations still batches exactly as before.
- Long prefills now re-choose their chunk width at every chunk boundary instead of holding the width they were admitted at. A 1M-token prefill on Qwen 3.8 Flash Next runs for minutes across a couple of hundred chunks, and the memory it was admitted against can move in that time — another request decoding between its chunks, a process outside the server taking RAM, or the indexer history and checkpoints the prefill itself accumulates. It now takes one cheap reading of live memory at each boundary and narrows the next chunk if the one it just ran no longer fits, which turns what used to be an uncatchable abort into a slower prefill that finishes. It widens again only when two consecutive readings agree there is room to spare, never after it has narrowed once, and never past the width the architecture would run anyway. The admission decision is untouched: a request is still billed at the width it was admitted at, and on a quiet machine serving one request nothing changes. Qwen 3.8 Flash Next only; `MLX_SERVE_PREFILL_CHUNK_ADAPTIVE=0` restores the single per-request width, and an explicit `--prefill-chunk` still wins outright.

- Long-context servers prefilled every prompt at the narrow chunk. With `--ctx-size 1048576` the prefill width was sized once at load, against the whole configured session's KV — 20.7 GB of a 28.2 GB budget on a 137 GB Mac — so the widest affordable width was 1024 tokens and a 4k prompt got the same treatment as a 1M one. Qwen 3.8 Flash Next now picks the width per request, at admission, where the estimator already knows how much KV that particular prompt holds: an ordinary prompt gets the wide chunk while a genuinely enormous one still steps down. Same estimator decides both, so a request can never run wider than it was billed for, and the admission check now walks the same ladder downwards before giving up — a prompt that only fits at the narrowest width is served instead of refused, which matters most for the long-but-not-maximal prompts where the widths differ. A refusal now names the width it gave up at. `MLX_SERVE_PREFILL_CHUNK_PER_REQUEST=0` restores load-time sizing, an explicit `--prefill-chunk` still wins outright, and every other architecture is unchanged.

- The prefix cache budget depended on how busy the Mac was at startup. Two runs of the same build with the same flags, eleven minutes apart, gave the same `--prefix-cache-mem 10GB` request 1,076 MB and 9,757 MB — a nine-fold difference, because the budget was measured against free system RAM at that instant and one run followed a test suite. The budget is now computed from the machine's fixed GPU working-set limit, so it is the same on every boot; free memory still governs individual requests, where the server can evict cached prefixes to make room. A boot under memory pressure logs the pressure instead of shrinking the cache for it.

- Raising the GPU memory limit could shrink the prefix cache. On the same machine and model, `--prefix-cache-mem 10GB` resolved to 3,873 MB at the default limit but only 1,076 MB after `iogpu.wired_limit_mb` was raised by about 10 GB: the extra headroom let the sizer pick a wider prefill chunk, and that chunk's reserve grew faster than the headroom did, so the cache paid for the upgrade. On Qwen 3.8 Flash Next, where each request now prices its own prefill width at admission, the load-time reserve is only a promise to the first request — so it is now taken at the narrowest width, and a bigger limit can no longer buy a smaller cache. An explicit `--prefill-chunk` is still reserved for as-is, and other architectures are unchanged.

- A generous `--prefix-cache-mem` could cut the context the server advertises down to 1,024 tokens. The auto-context sizer subtracted the whole requested cache budget from what it had to work with, even though the request had already been clamped to a fraction of it — so on a boot with no `--ctx-size`, asking for 60 GB of prefix cache left nothing to report and the server advertised a 1,024-token context for the rest of its life. Agent CLIs read that number once and budget against it for a whole session. The sizer now reserves the budget the cache was actually given. Boots that pass `--ctx-size` were never affected; the `max_safe_context` figure in `/props` was, on every boot.

- Asking for a bigger prefix cache could give you a smaller one. `--prefix-cache-mem 10GB` and `--prefix-cache-mem 24GB` on the same machine and model resolved to 3,873 MB and 5,703 MB of cache respectively: the requested budget was fed into the prefill-chunk sizer, a bigger request narrowed the chunk, a narrower chunk reserved less memory, and the cache was then handed what the chunk left over. The chunk is now sized without looking at the request at all, so the resolved budget only ever goes up as you ask for more. Two related fixes ride along: a boot with no `--prefix-cache-mem` can no longer size a prefill chunk that leaves the cache with nothing (it steps down instead), and an explicit `--prefill-chunk` is now the width the memory bill reserves for, not just the width the forward runs at — the two had drifted apart by several GB. The `[hot-cache] budget clamped` line now names the chunk and reserve it billed.

- A Metal working-set out-of-memory during a very long prefill killed the server instead of failing one request (#353). mlx-c's default error handler calls `exit(-1)`, so the first such error took every in-flight request with it, with no status line and no log entry. MLX errors are now caught and returned as a named memory 503; `MLX_SERVE_MLX_ERROR_LATCH=0` restores the old fatal behaviour.
- A long prefill could also be refused, or run out of memory, for capacity it was never going to need twice (#353). Past 32k tokens a request now reserves its cache up front, so the cache grows exactly once and no old-plus-new pair is ever live (`MLX_SERVE_KV_RESERVE=0` restores growth-on-demand); the admission estimate bills the restore points and indexer state it had been ignoring; and a request that does not fit now evicts cached prefixes to be admitted, refusing by name only when the cache is empty. A 512k-token cold prefill completes on a 128 GB M5 Max where it previously killed the process.
- A very long prompt sent with no `max_tokens` was admitted and then refused after paying for the prefill (#353 follow-up). An omitted `max_tokens` means "generate until the context runs out", and the cache reservation took that literally — a 383k-token prompt on a 1M context reserved 13 GB for a generation the request was never going to run, on top of the prompt it actually needed. The reservation now buys the prefill's own coexistence plus a bounded generation headroom, and the rest of the reply grows between decode steps as it always did. In the same pass the admission guard stopped crediting itself cache it cannot actually reclaim: a restored prefix is shared with the entry it matched, so counting that entry as evictable promised memory the eviction then had to protect. A prompt that genuinely does not fit is now refused before the prefill starts, with a named 400 instead of an out-of-memory 503.
...
- Flash Next's saved draft-head history was usually rejected the moment it was used, undoing the cache fix above. The head keeps its own row count, but the attention cache it writes into only counts rows for the model's first layer, and the head's layer is its last — so the saved history was almost always stamped with a length of zero and either refused by name (`MTP head restore declined`) or, worse, skipped without a word, leaving the head guessing on a full cache hit. Whether it happened depended on whether the previous reply ended with a draft to discard, which is why it surfaced only after unrelated timing changes. The head now stamps its own count, both the save and the restore refuse a mismatch by name, and a skipped restore always says so in the log.
...
- A hybrid-model prefix too big for the cache budget was dropped whole instead of being kept in part, so a long session went back to re-prefilling every turn (a 383k-token entry logged `skipped oversized entry (383069 tokens, 8757.79 MB > 3873.54 MB budget)`). The restore points a long prompt keeps were thinned by dropping the oldest, which left them all bunched at the end of the prompt, so the shortest prefix the cache could have kept was already too big to afford; they are now thinned from the middle, keeping the earliest and the newest, and the budget check no longer charges for restore points the commit discards anyway. A commit that still cannot keep anything now says which of the three reasons stopped it, and a failed copy retries at the next-lower restore point instead of giving up on the entry.
...
- The SSD-first prefix cache, the warm-turn admission bill and the chunk sharing above were reviewed and audited before release. Eight defects found that way were introduced and fixed inside this unreleased version and never reached a user, so they have no entries of their own; the user-visible shapes they settled are worth stating once. `--prefix-cache-mem` is a hard cap on the conversations that are not being served, and `0` keeps none of them in memory. A conversation leaves memory only once a durable copy is on disk and the index agrees it is there — a write that was skipped for any reason no longer counts as one. SSD-first needs a disk tier to arm at all, so `--prefix-cache-disk` being off means the mode is off. An entry that inherits chunks from another takes only the chunks that have actually landed on disk, and never disturbs the other entry's pending writes. And the per-chunk prefill width is gated on the architecture, not on a hook the server installs for every model, so it stays off everywhere but Flash Next.

- The split-K kernel above is exact to two bf16 ulps in the hermetic tests and gives the same answer at 1, 8, 16 and 64 splits. An end-to-end greedy check at 62.7k tokens is reported here rather than as a pass mark, because on that prompt it does not discriminate: upstream `main`, with none of this work, already puts 4 of 8 prompts over the 0.15-nat first-divergence bar (the kernel puts 3 of 8 over it). The flips are the prompt's own near-ties, so the kernel is judged against upstream rather than against an absolute bar.
