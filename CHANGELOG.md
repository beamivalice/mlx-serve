...
- Long-context servers prefilled every prompt at the narrow chunk. With `--ctx-size 1048576` the prefill width was sized once at load, against the whole configured session's KV — 20.7 GB of a 28.2 GB budget on a 137 GB Mac — so the widest affordable width was 1024 tokens and a 4k prompt got the same treatment as a 1M one. Qwen 3.8 Flash Next now picks the width per request, at admission, where the estimator already knows how much KV that particular prompt holds: an ordinary prompt gets the wide chunk while a genuinely enormous one still steps down. Same estimator decides both, so a request can never run wider than it was billed for. `MLX_SERVE_PREFILL_CHUNK_PER_REQUEST=0` restores load-time sizing, an explicit `--prefill-chunk` still wins outright, and every other architecture is unchanged.
