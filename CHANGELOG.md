...
## Highlights
- **The opencode2 monitor meters turns served by another provider from their own stream, and its prefill bars no longer run off the sidebar.** A session on a cloud or remote provider stops borrowing this Mac's throughput numbers — its prefill is timed to the provider's first streamed content and its decode read from the stream — while local sessions keep the live feed under either provider id the launcher writes; prefill bars cap at 12 blocks and the sidebar Throughput line draws the rate it names. The plugin itself is now vendored in this repository at `lib/opencode2-mlx-serve` instead of being pinned as a submodule, so a plain clone builds and ships it.
